...
- **The full-catalog fallback did not run in the case it exists for.** §7.0 makes the fallback the deliberate backup for bad descriptions: when the planner reports no confident match for a capability-needing query it "takes over **rather than answering blind**", and every fallback use is traced with route rung `fallback`. `plan_node` computed `use_fallback` correctly from `no_confident_match` — but `after_plan` tested the `direct_answer` short-circuit **first**, and a helpful model almost always writes a courteous refusal alongside the flag ("I don't have an invoice-reconciler capability, so I can't route this request"). That refusal won, the fallback was discarded, and the run answered blind with no `route` step at all. The ordering is reversed; the multi-part behaviour added in `b97e1ca` — a direct answer accompanying real entries still resolves and merges — is pinned by its own test so the reorder cannot undo it. **Found by a live acceptance stage, not by the suite**: the existing `test_no_confident_match_uses_full_catalog` passes `no_confident_match=True` with *no* direct answer, so the short-circuit never fired and the ordering bug hid behind it. `after_plan` had no test of its own at all; it now has four, plus two integration tests for the shape a real planner actually emits.
...
- **"A HITL-paused run is invisible in the chat UI" was wrong, and the cause was the test harness.** Four trial stages failed to find their approval card while the runs sat genuinely `paused_hitl`; `/hitl/pending` listed them, the SSE replay carried the `hitl_request`, and the card still never appeared. The failure frame shows the chat empty but for `ENGAGING ORCHESTRATOR_` while the server had already emitted twenty events. The defect was in the throwaway proxy standing in for nginx in the Docker-free bring-up: it pumped the upstream response with `read(8192)`, which blocks until the full buffer fills, so an event stream that trickles a few hundred bytes per event delivered **nothing** to the browser until 8 KB had piled up. Every live card — plan, rails, gate — stayed invisible. With `read1()` the gate renders, the Approve button is live, and reopening a conversation re-attaches to its paused run and replays correctly, exactly as `ChatPage` intends. No product change was needed and none was made.

  The repair then broke the harness a second way, which is worth recording next to the first: serving SSE unbuffered meant moving the proxy to HTTP/1.1, and its error path sent neither `Content-Length` nor `Connection: close`, so under keep-alive every 4xx hung waiting for a body the browser already had. A 422 the API answered in 235 ms became a two-minute stall, and stage `04` regressed from passing in 16.9 s to timing out — a *second* run discarded chasing a *second* defect that was not in the product.
...
**A second pass over the repaired tree found four more, three of them mine.** The `finally` that was supposed to stop stage `28` from throttling the deployment was itself rate-limited — `api()` returns rather than throws, so the 429 was swallowed and the limit stayed at 1 req/s, reddening six stages exactly as before. It now retries until the bucket refills and says so if it cannot. The theme fix patched the first of **two** copies of the same `data-theme` assumption. The widened refusal regex was still too narrow for "There is no file named `does-not-exist.txt`" — and note that `not exist` could never have matched that hyphenated fixture filename anyway. Matching live-model prose is brittle by nature, and the assertion says so now.

**How often the HITL gate is reached is now measured, not assumed.** The `site-analyst` workflow's branch conditions are natural language the model evaluates, so whether a run reaches the `approve` gate or takes the "if nothing to do" edge is a model decision. Over one clean campaign it reached the gate on **three trials of four**. Stages whose subject is the gate (`06`–`09`, `11`) now re-ask on a fresh conversation up to three times, the same pattern stage `35` already used for planner routing, and log each attempt rather than reporting a routing coin-flip as a broken gate.

**An intermittent cross-file test flake, recorded rather than hidden.** One full-suite run ended `1 failed, 1275 passed` on `test_memory_episodic.py::test_block_approved_instructions_get_their_own_section`; the immediately following run of the same code in the same order ended `1276 passed`. It passes alone, in its own file, and adjacent to the tests added this wave, and it collects *before* them, so nothing here caused it. It is order-dependent state leaking from somewhere earlier in the suite. Not diagnosed, not fixed, and named here so the next green run is not mistaken for proof it is gone.
...
- **Whether the trial stages reach their HITL gate is not yet established.** Across the discarded runs the gate armed once and was missed three times, by two different routes — once the orchestrator resolved the task on the ladder without dispatching to the sub agent at all, once the sub agent's own branch took the "if nothing to do" edge. The gate machinery itself is proven working. What is not yet known is how reliably the model's routing decisions line up to reach it, and no claim is made here either way, because every one of those observations was made through a proxy now known to have been broken.
...
- **The backend container now runs as a non-root user** (`concierge`, uid/gid 1000). A `workspace` volume created by a root-era image is not re-chowned automatically. Run once, before the first `./start.sh` on the new image:
  `docker compose run --rm --user 0 --entrypoint chown backend -R 1000:1000 /workspace`
  A fresh volume needs nothing.
- **The frontend container's internal port moved from 80 to 8080** so its nginx master can run unprivileged. `docker-compose.yml` publishes `${FRONTEND_PORT:-5173}:8080` — **the published host port is unchanged**. Anything that targeted the container's port 80 directly (a custom proxy, a Kubernetes `targetPort`, a healthcheck) must move to 8080.
- **The registry cache default is now `memory`**, not `bypass`.
- **A request rate limit and a body-size cap are now unconditional.** They previously lived behind auth, which ships dark. A client that sustained more than `rate_limit_per_s` (10) will now see 429s; the load harness raises both keys for its own run and restores them.
- `AUTH_SESSION_TTL_H` is finally passed into the container; a deployment that set it in `.env` and relied on the silent 24 h default will now get the value it asked for.

### Fixed

- **Deactivating an MCP server or a remote agent was a label.** `status='inactive'` was overloaded: a server is *born* inactive and its first successful connect flips it to active, so the status alone could not distinguish "not connected yet" from "a human switched this off" — `start()` and the reconnect loop connected it again every interval, and a failed health ping wrote `error` over the disable, after which the next success wrote `active`. Both registries now carry a `disabled_at` column recording the operator's decision, honoured by connect, reconnect and card refresh, never overwritten by a status write, and surviving a restart. A skill arriving on a disabled agent's card is born out of service rather than active and callable under a parent nobody enabled.
- **The spend ceiling counted runs only.** Every model call the system made on its own initiative — the overlap, significance and salience judges, anticipation, run digests, reflection, community summaries, extraction — sat outside it, so the Settings hint promised "one number for the whole deployment", the dashboard reported $0 for all of that work, and an operator at their ceiling watched chat get refused while the background jobs kept billing. A `job_usage` ledger records one row per out-of-run call, priced when it happens exactly as a run is, and each of those jobs checks the same ceiling at its entry point (declining quietly and retrying next tick, since nobody is waiting on it).
- **Registering a stdio MCP server is registering a subprocess**, and the endpoint accepted any command. Launchers are now allowlisted by basename (`uvx`, `npx`, `uv`, `node`, `python`, `python3`, plus `MCP_STDIO_ALLOW`), refused at the form with a 422 rather than as a connection error minutes later — on `POST` **and on `PATCH`**, which was the hole that made the guard decorative: register with `npx`, then rewrite the command.
- **A routine's allowlist was decorative in places.** It is now enforced before ranking in the graph-mode planner catalog and all three registry middlewares, *and* at the resolution ladder, so a capability id carried in a checkpoint from before the allowlist was narrowed cannot get in through the side door. Because that is a silent behaviour change for a routine whose allowlist was written when it did nothing, boot runs a read-only audit naming every routine whose recent runs used something the ceiling would now refuse — before it next fires, rather than inside an autonomous run nobody is watching.
- **A validation error echoed the request body back**, so a rejected `PATCH /settings` carrying a credential returned it in the error payload. The echoed `input`, `ctx` and `url` are stripped.
- **Nothing bounded a request body** anywhere — not the app, not uvicorn, not nginx — so one large POST was an out-of-memory kill of the single process that serves everything. Capped on the declared length *and* while the body streams, so a chunked request cannot walk past it.
- **A first boot keyed only with `OPENROUTER_API_KEY`** — the provider every acceptance frame in this repository was captured on — matched nothing in the first-boot preference list, kept the unconfigured `anthropic:claude-sonnet-4-6` code default, and could not run a single chat until a human opened Settings. `openrouter` and the `custom` gateway are now in the list, the latter resolving to the first model it actually declares.
- **The eval harness auto-approved every human gate it reached**, so an uploaded one-case dataset pointing at a gated destructive workflow drove it to completion with the approval granted by a log line. Auto-approval is now per-dataset opt-in (`allow_hitl_autoapprove`, default false) and every machine-cleared gate is recorded as a step.
- **HITL resume was not single-shot** — two resumes of the same paused run both proceeded. The claim is now an atomic conditional update; the loser is told the run is no longer paused.
- **Two overlapping A2A card refreshes both inserted**, leaving an agent with duplicate tools and no way to tell them apart. One row per (agent, skill id) is now a database fact, with an on-conflict upsert.
- Settings validation: a boolean was accepted for every integer key (`True` is `1` in Python, so `max_plan_steps: true` stored a plan limit of one), `default_model` could be cleared even though it is the fallback for every role, quiet hours accepted any list length, and `HH:MM` fields accepted `99:99`.
- Nineteen foreign keys that every cascading delete walked with a sequential scan are indexed, as is the `thread_id LIKE '<run>:%'` prefix the per-run checkpoint purge does (`text_pattern_ops` — a normal index cannot serve it under a non-C collation).
- `concierge_memory_recall_seconds` was declared and never observed: the one latency the memory layer is judged on scraped as an empty series.
- Chart specs reaching the page from the tool path were unvalidated while the formatter path validated its own; non-finite numbers could reach the database; a numeric table cell discarded the entire document; charts nested in a card never rendered; and the SVG scaler was zero-anchored, so any negative value drew outside the viewBox.

### Changed

- **`registry_cache_mode` ships as `memory`.** `bypass` stays valid and remains the live rollback lever — but it is a *live read, not a faster cache*: invalidation is event-driven and exhaustive, so a cached read is current for the same reason a bypassed one is, and shipping `bypass` paid a full Postgres query on every model call for a guarantee the cache already gave. ADR-0004 records the supersession.
- **The `§17.3` chain-guard kill switch reads its setting.** `ambient_routine_events_per_hour` was displayed and editable in Settings while the guard used a hardcoded 50, so lowering the cap changed nothing. The constant is gone. The spec sentence claiming the guard auto-disables the routine was wrong and is amended: past the cap the event ingest is refused and the routine stays enabled.
- **Master switches are enforced inside the behaviour, not only at the caller** (§3.7.1) — eleven ambient tick evaluators and the memory job family each check their own master, so a direct call cannot bypass a switch the tick respects. Two deliberate exemptions are documented in code: the registry overlap audit is a §4 job that must run when memory is dark, and the backlog-depth gauge is a pure read that performs nothing.
- Retention gained three more born-dark targets — `runs`, `run_steps` and `checkpoints` — each with its own gate enforced inside the purge. A queued, running or paused run is never touched at any age, and a paused run's checkpoints are its resume path.
- Every LLM prompt this wave touched moved into `app/prompts/` with a golden set, including three new files (`fallback.md`, `prior_outputs.md`, `memory_sections.md`). The summariser's input is now fenced — `web-research` chains fetch → summarize, so a fetched page's own instructions used to arrive unfenced and the summary they produced was trusted downstream.

### Documentation honesty

- ~65 documentation findings were classified one by one and closed: the counts on `README.md` and `docs/README.md`, the acceptance frame and disposition totals, a rewritten code tour (15 modules and 5 packages were missing), a rewritten data model (40 tables, 29 migrations, the real `status` and `step_type` enums, the typed HNSW columns), the REST reference extended from 37 paths to all 86 paths / 115 operations, the configuration reference from 17 environment variables to the real 52 and from "28 settings keys" to 122, and the superseded claims about the SSE keepalive (15 s, not 120 s), stream resolution after a restart, purge and the checkpoint tables, the compiled-worker cache key, CORS, and the secret inventory.
- The spec said **"TTLs are forbidden"** in three places while `REGISTRY_CACHE_TTL_S` (300 s) expires every memory-mode entry, and `registry_cache.py`'s own docstring repeated it twenty lines above the constant. Corrected everywhere to what is actually true: invalidation is the mechanism; the TTL bounds the one case invalidation cannot cover, a cross-replica NOTIFY that was never delivered.
- §14's script is eleven steps, not ten, and its step 1 described a seed of "2 skills, 1 native tool" against an actual 5 skills, 10 tools and 3 sub agents.
- Four runbook errors fixed (`docker compose exec --index=N`, the `ambient_channel_failed` / `_retry` event names, a broken environment table, `BACKEND_PORT` vs `BACKEND_PORT_RANGE` across seven pages) and **seven runbooks added**: spend ceiling, MCP circuit open, LISTEN down, checkpointer outage, Redis outage, embedding backfill stuck, stalled run / wall clock. Where an operator would want a signal that does not exist — the checkpointer has no metric, no probe and no log event, and `/ready` ignores it — the page says so under **Known gap** rather than inventing one.

### Known limitations, stated rather than fixed

- **Ownership checks on cancel, HITL resolution, purge and the ledger are left to the auth workstream** and are documented as gaps, not closed here.
- **Single-replica only.** Ownership re-stamping, terminal-write guards, the leader lease and job-clock ordering are documented as known multi-replica limitations rather than fixed; the single-shot HITL resume above is the one exception that was.
...
- **Proof:** the whole ceremony re-run from a fresh volume on the rebuilt image (`docs/acceptance/prod/M56/`), the third-reading drill (`prod/HARDENING/third-reading.md`), stage 36 and the regression stages 02–05, 14, 18, 27, 28, 35 on that image; backend 1170 passed / 1 skipped, frontend 109.
...
- Full acceptance ceremony (spec §18.10: fresh volumes via `./decom.sh -y` + fresh `docker compose up` on qwen3.8-max, then the original §14 eleven-step script re-earned end-to-end — seed → UI-registered MCP server → custom skill → exposure toggle → branch/error/HITL sub agent with inline validation → multi-turn chat with mid-run HITL + history follow-up → uncovered-ask fallback rung → kill-server error-edge + reconnect → trace/cancel/retry → planner-label + second-provider (gemini-3.6-flash) switch + purge → agentic + mid-session tool live-sync; plus §14c ambient UI + decision plane live (typed builder, webhook fire matched/held, live qwen watch compile, correlation chain, run history), an adversarial fire proven fenced, a live custom-gateway smoke with usage metadata, and the §11 byte-identity/dark-mode suites — 66 UI frames + transcripts, captured at the time under `docs/acceptance/ceremony_m36/`; that tree has since been superseded twice and `docs/acceptance/prod/M56/` now holds the third-reading re-run of the ceremony and its addendum (11 frames, 4 transcripts), with the M36 frames in git history; full backend suite at M36 691 passed, 1 skipped)
