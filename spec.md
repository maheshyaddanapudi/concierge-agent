...
### 3.2 tools
- `kind`: `'mcp' | 'native' | 'a2a'` (§19.4 projects a remote agent's card skills into the tools registry)
...
- `tool_key` (string, unique **among live rows**) — human-facing identifier. Defaults to `{server_name}.{tool_name}` for MCP tools (collision-safe across servers exposing the same tool name) and the registration name for native tools; auto-generated uuid string only if neither yields a unique key. **Editable later.** Internal references (skill_tools join) always use the immutable `id`, so renaming `tool_key` never breaks bindings. Uniqueness is scoped to rows that are not soft-deleted (`ix_tools_tool_key` is partial on `deleted_at IS NULL`, and the ingest's collision probe filters to match): a deleted tool must not go on reserving its key, or deleting an MCP server and registering it again under the same name would collide with its own tombstones and suffix every key. The suffix is for two *live* servers exposing the same tool name — never for a server and its deleted past. The cost of reuse is that restoring a soft-deleted tool whose key a live row has since taken is refused with **409**; the operator renames one and retries, and nothing is silently suffixed on their behalf.
...
- Every mutating endpoint bumps `updated_at`; registry reads go through the §7.3 `RegistryCache`, whose freshness contract is event-invalidated (`bypass` mode makes them literal live DB reads and is the rollback lever).
...
- **`bypass`** — no state; every read executes the same Postgres queries as before the cache existed. Byte-identical semantics; the rollback lever. It is a **live read, not a faster cache**: the name describes what it skips, and a deployment on it pays the full query cost on every model call.
- **`memory` (default since code_setting_ui_hardening)** — per-process store loaded on startup, per-registry generation counters, **reload-on-dirty** (an invalidation marks the registry stale; the next read reloads it wholesale — registries are small, and full reload cannot leave stale embedded relationships). This is the mode the design was built for: invalidation is event-driven and exhaustive (below), so a cached read is current for the same reason a bypassed one is, and shipping `bypass` paid a full Postgres query on every model call for a guarantee the cache already gave. The `REGISTRY_CACHE_TTL_S` backstop bounds the one case invalidation cannot cover — a lost cross-replica NOTIFY — at 300 s. `bypass` remains one setting away and applies live.
...
- **API guardrails (M40)**: rate-limit burst + refill per second (`rate_limit_burst` / `rate_limit_per_s`) — admin-gated like every settings write. Since code_setting_ui_hardening these shape a limiter that is **unconditional**: it used to live inside the auth middleware behind `provider.enabled()`, so the shipped configuration — auth dark — had no limit at all. It now runs always, keyed on the caller's address, with the per-principal bucket applying on top when auth is on. A request body cap (`MAX_REQUEST_BYTES`, and `MAX_UPLOAD_BYTES` for the §15 dataset upload) is enforced in the same middleware on the declared length AND while the body streams, so a chunked request cannot walk past it; over the cap is a 413.
...
| M8 | Answer surfaces, form gates, and provider campaigns (§5b/§7.1/§3.3): markdown-rendered canonical answers with a collapsible structured-summary toggle; HITL **form gates** — workflow gates carry choice and text questions, answers recorded verbatim on the `hitl` step and delivered into worker state; themed pure-SVG charts (`bar`/`line`/`pie`) + the `render_chart` native tool; per-skill `max_tool_iterations` loop budgets (`web-research` ships 20); OpenAI reasoning-effort runs routed through the Responses API entirely inside the adapter, zero consumer changes (ADR-0007) | provider agnosticism end to end — five conversations × both orchestrator modes on `openai:gpt-5.6-terra`, a Gemini bonus run, heterogeneous three-provider single runs; acceptance stages 19–21 |
| M9 | The formatter (§7.1): an explicit presentation role beside planner/aggregator — `formatter_enabled` (off = the call never runs, raw renders directly, no artifact exists), `formatter_model`/`_model_params`, `formatter_presentation` (`a2ui_first` \| `raw_first`), charts toggle, `formatter_coverage_flag_threshold`; `prompts/formatter.md` with a binding parity contract (preserve every fact, number, warning, recommendation; drop only duplication); artifact-driven, run-time-frozen rendering (`runs.answer_ui` carries its own presentation + coverage); a deterministic coverage metric computed in code as a quiet badge, never a render gate; formatter-independent charts persisted on `runs.charts` | presentation is a switchable role, not a hardcoded shape — history renders by what happened at run time, never by current settings; acceptance stage 24 |
| M10 | Direct sub agent invocation (§7.5): `sub_agents.direct_exposure` + four surfaces on one path (`POST /sub-agents/{id}/invoke`, `POST /chat` with `target_sub_agent_id`, the Sub Agents row action, the composer target picker); the `direct` orchestrator mode is a one-node graph on the run's checkpointer thread reusing the ladder executor, so HITL pause/resume, step recording and a pinned `route` step are identical to routed dispatch; gating (active + exposed) enforced at the API (403/409) and re-checked at execution start. Plus the seeded native tier (§9): `workspace-auditor` + `workspace-curator` skills, and `workspace-warden`, the first native sub agent | the planner is bypassable without a second execution path — every §7.5 surface works on native, custom and file-defined agents alike |
| M11 | Opt-in history summary for direct invocations (§7.5): `include_history_summary` — direct runs stay cold by default; one summarization call (default model, effort low, `prompts/history_summary.md`) recorded as a `summary` step with usage rolled up, fail-open to the cold task; the task is checkpointed so a HITL resume never re-summarizes and retry preserves the flag; 422 on `/chat` without a target and on `/invoke` without a `conversation_id` | the translator role the planner plays for routed dispatch is available to a pinned agent — opt-in, traced, cold by default; acceptance stage 27 |
| M12 | Declarative `.agent.md` sub agents (§3.4): a third authoring path onto the same `sub_agents` row — YAML frontmatter (name, description, persona, model/model_params, direct_exposure, §3.5 workflow) + a documentation body. Skill nodes reference skills by NAME; the seed pass resolves them to registry uuids, validates with the same structural + factory-compile checks the API applies at save, and upserts keyed on (name, static) with file provenance in `native_ref`. Invalid files land `status='error'` (UI-visible, logged, never crash boot); user status/exposure toggles survive reseeds while definition fields always follow the file | all three authoring paths converge on one registry row — ladder, planner catalog, direct invocation, UI rendering and static rules work untouched |
...
| M36 | Full acceptance ceremony: fresh volumes, fresh `docker compose up`, the eleven-step script + §14c-20..31 top to bottom + channel/eval/gateway/auth stages; final report | the definition of done, re-earned end-to-end |
...
| M56 | Release (PLAN M56): LICENSE (MIT), CONTRIBUTING, SECURITY with the auth-is-a-fork stance, issue and PR templates, the CHANGELOG reconstructed from the README milestone table under `v1.0.0`, version `1.0.0` in `backend/pyproject.toml` and `frontend/package.json`, the README retold as run-and-extend; the §14 script (steps 1–11) on a fresh `docker compose up` of the release images with a UI frame per step, an addendum for the DAG error edge and the agentic todo list, and the M49 load scenarios re-run as the performance record | the definition of done, published — `docs/acceptance/prod/M56/`; no git tag by decision (no consumer yet, the version lives in the files) |
...
1. Open admin → seed visible: 2 stdio MCP servers (`fetch`, `filesystem`, registered inactive until they connect), tools with `static` badges, 5 native skills, 10 native tools, and 3 sub agents — `research-concierge`, the declarative `workspace-reporter`, and the native `workspace-warden`.
...
if its own id appears in its causation chain (**no self-trigger**); a kill
switch at `ambient_routine_events_per_hour` fires/routine/hour — the live
setting, not a constant (code_setting_ui_hardening: the guard read a
hardcoded 50 while Settings displayed, and let an operator edit, a key
nothing consumed, so lowering the cap changed nothing). Past the cap the
ingest is **refused** — the event is not recorded and the caller is told
which routine tripped it and at what count. It is a back-pressure guard, not
an auto-disable: the routine stays enabled and admits again in the next
hour, because the runaway a chain guard catches is usually a burst, and
disabling a routine is an operator's decision to reverse, not a counter's.
Per (rule, partition) cooldown default 300s. Routines may not wake themselves
except via the capped wakeup tool.

### 17.4 Execution plane

**The allowlist is a real ceiling, in both modes** (code_setting_ui_hardening). The routine's narrowed projection is applied by `apply_ambient_allowlist` before ranking — in the graph-mode planner catalog and in all three registry middlewares — AND the resolution ladder refuses an entry outside it, so a capability id carried in a checkpoint from before the allowlist was narrowed, or named by a pinned plan, cannot get through the side door. A kind the allowlist does not mention stays unconstrained. Because that ceiling was previously decorative for some routines, boot runs a **read-only allowlist audit**: for each active routine with an allowlist it reads what the routine's recent runs actually used and logs, by name and with the entries listed, every routine the ceiling would now refuse — once, at startup, so an operator widens the allowlist before the routine next fires rather than after an autonomous run nobody watched has already been refused. Advisory: it never edits a routine and never fails or delays boot.
...
provider. Scope, in order: the original §14 eleven-step script; §14c steps
