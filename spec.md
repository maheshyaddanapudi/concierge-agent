...
| Sub agents | `GET/POST /sub-agents`, `GET/PATCH/DELETE /sub-agents/{id}`, `POST /sub-agents/{id}/validate` (dry-run factory compile), `POST /sub-agents/{id}/invoke` (`{message, conversation_id?}` → `run_id` — direct invocation, §7.5; 403 unless `direct_exposure`, 409 unless active), `POST /sub-agents/check-overlap` (pre-save judge, below) |
| Chat | `GET/POST /conversations`, `GET /conversations/{id}` (messages + runs), `POST /chat` (`{conversation_id, message, target_sub_agent_id?}` → `run_id`; the optional target pins the run to that sub agent — direct invocation, §7.5, same gating as `/invoke`), `GET /chat/stream/{run_id}` (SSE), `POST /runs/{run_id}/hitl` (`{"decision": "approve"|"deny", "note": "..."}`) |
