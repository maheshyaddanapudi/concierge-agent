...
`last_error`, recovery restores `active` — an operator's `inactive` is never undone by a refresh (the loop skips disabled agents, and the call path refuses one; hardening wave). `card_hash` / `card_version` (hardening wave): a card that differs under a refresh bumps the version, logs the changed keys (`a2a_card_changed`) and counts (`concierge_a2a_card_changes_total`). `remote_agents` is manager-held
