...
export default async function ({ page, nav, shot, get, log, click, closeDrawer, submitSave }) {
  ...
  const lastSkill = page.locator('select').filter({ hasText: 'pick skill…' }).last()
  await lastSkill.selectOption(notes.id)
  await click(page, '+ edge')
  await page.waitForTimeout(300)
  const edgeRows = page.locator('div').filter({ has: page.getByPlaceholder('condition (optional)') })
  const last = edgeRows.last()
  const selects = last.locator('select')
  const ids = (await page.locator('select option').allTextContents())
  const firstNode = ids.find((t) => /^n1$/.test(t)) ? 'n1' : null
  const nodeCount = await page.getByPlaceholder('condition (optional)').count()
  const newNode = `n${nodeCount}`
  await selects.nth(0).selectOption(firstNode || { index: 1 })
  await selects.nth(1).selectOption(newNode).catch(() => selects.nth(1).selectOption({ index: 1 }))
  await selects.nth(2).selectOption('error').catch(() => {})
  await page.waitForTimeout(300)
  await shot(page, '00-builder-error-edge')

  // a dangling edge is refused inline
  await click(page, '+ edge')
  await page.waitForTimeout(300)
  await click(page, 'Validate (dry-run compile)')
  await page.waitForTimeout(900)
  await shot(page, '01-validation-rejected')
  // remove the dangling edge (the last "✕" in the edges list)
