...
as() { echo "-H Authorization:Bearer\ $1"; }
...
curl -s $API/auth/me $(as $TOKEN) | py 'print("GET /auth/me →", d)'
say "§3 user admin: admin creates member 'mallory'; she logs in with her own credentials"
curl -s -X POST $API/auth/users -H "$H" $(as $TOKEN) -d '{"username":"mallory","password":"mallory-pass-1","role":"member"}' -w ' → HTTP %{http_code}\n' | cut -c1-160
...
curl -s -X PATCH $API/settings -H "$H" $(as $TOKEN) -d "{\"default_model\":\"$ACC_MODEL\",\"orchestrator_mode\":\"graph\"}" -o /dev/null
RID=$(curl -s -X POST $API/chat -H "$H" $(as $TOKEN) -d '{"message":"In one sentence: what is a bearer token?"}' | py 'print(d["run_id"])')
for i in $(seq 1 180); do st=$(curl -s $API/runs/$RID $(as $TOKEN) | py 'print(d.get("status",""))'); case "$st" in completed|failed|cancelled) break;; esac; sleep 1; done
curl -s $API/runs/$RID $(as $TOKEN) | py 'print("run", d["status"], "|", (d.get("final_answer") or "")[:110])'
...
curl -s "$API/runs?limit=50" $(as $MTOKEN) | py 'print("mallory GET /runs →", len(d) if isinstance(d,list) else d, "runs")'
curl -s $API/runs/$RID $(as $MTOKEN) -o /dev/null -w 'mallory GET /runs/{admin run} → HTTP %{http_code}\n'
say "§6 ambient ownership: mallory's routine, invisible to admin; the fire token is the only auth on the fire path"
curl -s -X PATCH $API/settings -H "$H" $(as $TOKEN) -d '{"ambient_enabled":true,"ambient_tick_interval_s":15,"ambient_quiet_hours":[]}' -o /dev/null
RT=$(curl -s -X POST $API/routines -H "$H" $(as $MTOKEN) -d '{"name":"mallory-ping","prompt":"Reply with exactly: pong","autonomy":"propose","triggers":[{"type":"webhook"}]}' | py 'print(d.get("id",""))')
echo "mallory routine $RT"
curl -s $API/routines $(as $TOKEN) | py 'print("admin GET /routines →", [r["name"] for r in d])'
curl -s -X POST $API/routines/$RT/token $(as $TOKEN) -o /dev/null -w 'admin mints a token for it → HTTP %{http_code}\n'
FT=$(curl -s -X POST $API/routines/$RT/token $(as $MTOKEN) | py 'print(d.get("fire_token",""))')
