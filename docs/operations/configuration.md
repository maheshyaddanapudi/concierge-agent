...
| `SHUTDOWN_GRACE_S` | `25` | M51: on SIGTERM the process flips `GET /ready` to 503, refuses new runs (503 + `Retry-After`), waits this long for in-flight runs to finish, then cancels the remainder — each ends `cancelled` with an error naming the shutdown and the grace. The next boot reaps anything still `running`/`queued` as "orphaned by a restart". Keep it under the orchestrator's stop timeout (compose `stop_grace_period` is 30 s; Kubernetes `terminationGracePeriodSeconds`). Note that uvicorn closes the listening socket at SIGTERM *before* the lifespan drain runs, so during a `docker stop` clients see connection-refused rather than the 503 — the 503 is what a pre-stop hook or a load balancer probing before SIGTERM sees (the deploy-lifecycle hook itself lands in M53). | Restart |
