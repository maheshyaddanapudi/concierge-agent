...
Eight gates are born off because deleting is irreversible; the expired-session sweep is on because the login path already did it opportunistically. The job runs hourly from the periodic loop on one replica (advisory lock `427018`), deletes in batches of 5000 — 500 for the three run-ledger tables, since each run drags its steps and its checkpoints with it (`_BATCH` / `_RUN_BATCH` in `app/retention.py`) — and counts what it removed in `concierge_retention_deleted_total{table}`. Settings → Retention shows each gate, each window, and **how many rows a purge would delete right now** (counted whether the gate is on or off), with a run-now button; `GET /retention` and `POST /retention/run` are the same surface over the API.
