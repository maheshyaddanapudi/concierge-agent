...
**What it is.** `spend_ceiling_enabled` puts **one** USD ceiling across every
kind of model work — chat runs, direct invocations, ambient fires, eval
batches, and the out-of-run jobs (judges, digests, reflection, community
summaries, extraction, embeddings). Spend is summed from the database over
...
| `concierge_spend_ceiling_refusals_total{kind}` | flat | climbing, labelled by what was refused (`chat`, `direct`, `ambient`, `eval`, or a job class) |
...
# and the half that is not a run (judges, digests, extraction, embeddings)
docker compose exec db psql -U concierge -d concierge -c \
...
2. **One job class is eating the budget** — `by_kind` in `GET /spend` (and
   the `job_usage` query above) shows one `kind` dominating: `reflection`
   over a large store, `community` rebuilding, `embedding` backfilling a
   fresh model key, `significance` / `salience` / `anticipation` on a busy
   ambient tick, `overlap_audit`, `eval_judge`.
