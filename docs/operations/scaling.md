...
- **The database service declares `shm_size: 1gb`** (`docker-compose.yml`). pgvector index builds and parallel plans use POSIX shared memory; Docker's 64 MB `/dev/shm` default fails them at scale with `could not resize shared memory segment … No space left on device`.
