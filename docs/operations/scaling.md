...
The pooled ceiling per backend replica is explicit: `DB_POOL_SIZE` (5) + `DB_MAX_OVERFLOW` (10) SQLAlchemy connections, with `DB_POOL_TIMEOUT` (30 s) the wait before a request fails. Outside that pool each replica also holds the LangGraph checkpointer pool (up to 10, psycopg) and **four** session connections — one LISTEN for cache invalidation, one for the ambient drain's wake channel, the control listener, and the ambient leader lease — budget **14** for those. Size Postgres `max_connections` from `replicas × (pool + overflow + 14)` plus a 10-connection reserve for migrations, `psql`, and the load harness; the full arithmetic, and the `GET /replicas` endpoint that publishes it, are in [the connection budget](#the-connection-budget) below. Two things that used to eat the pool no longer do: `/chat/stream` releases its session before streaming (the M49 baseline measured the pool exhausted at 15 open tabs), and `/ambient/stream` never had one. Since M51 no session is held across a provider round trip (the drain claims and commits before it processes; memory writes embed between sessions), so a burst's connection demand is bounded by its write bursts, not by provider latency.
...
Three more things the million-row drill taught, each shipped:

- **Recall has two legs and only one of them is flat.** The vector leg is an HNSW index scan whatever the corpus size. The lexical leg (`ts_rank_cd` over the GIN index) has to *visit every row that matches any query term* to rank it, so its cost is proportional to the match count — on the drill's synthetic corpus, where three of eight topic words appear in every row, that is 37% of the table (375k rows at 1M, ~1.1 s). Real corpora match far fewer rows per query, but the shape is the same: recall latency at scale is bounded by lexical selectivity, not by the vector index. The M54 recall transcript reports the two legs separately.
- **`memories.supersedes` / `superseded_by` are indexed** (migration `t9h0i1j2k3l4`). They are self-referencing foreign keys; without indexes every deleted memory made Postgres scan the table for rows pointing at it, and the drill's one-million-row cleanup stalled for eleven minutes before it was cancelled — 31 s with the indexes. Erase (§16.1) walks the same path.
- **The database service declares `shm_size: 1gb`** (`docker-compose.yml`). pgvector index builds and parallel plans use POSIX shared memory; Docker's 64 MB `/dev/shm` default fails them at scale with `could not resize shared memory segment … No space left on device`. The same build peaked at ~860 MiB of the database container's 1 GiB `deploy.resources.limits.memory` for a million 64-dimension vectors and ran an hour on two parallel workers; at that scale raise the database limit with the corpus (the graph must fit `maintenance_work_mem` *inside* the container's limit) and expect index builds to be a maintenance window, not a tick.
