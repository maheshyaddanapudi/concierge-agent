...
| `node '<id>' question '<qid>': choice needs an 'options' list with >=2 entries` | Choice with fewer than 2 options (or a scalar `options:`). |
| `node '<id>' question '<qid>': choice options must be …` / `duplicate choice options` | Non-string or repeated options. |
| `node '<id>': question ids must be unique` | Duplicate question ids. |
| `node id 'START' is reserved` (or `'END'`) | Reserved id used for a node. |
| `node id '<id>' is reserved (ids may not be __dunder__ or start with '__route__')` | A node id that would collide with LangGraph's `__start__`/`__end__` sentinels or with the synthetic `__route__<id>` node the factory generates per node. |
