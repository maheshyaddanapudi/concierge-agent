...
- History is **in-memory, per process** and bounded (M51: finished runs are evicted after 15 min, at most 500 runs). **When the process holds no history for a run** (a deploy, a restart, an eviction) and the run's row is terminal or paused, the server **synthesizes the terminal events from the row** (M53, `synthesize_terminal_events`): `run_status: completed` + `done` carrying the recorded answer for a completed run, `error` + `run_status: failed` for a failed one, `run_status: cancelled` for a cancelled one, `run_status: paused_hitl` for a paused one — each with `"replayed_from": "record"` in the envelope and `seq` continuing from the client's `Last-Event-ID`, so folding stays idempotent. These four are the whole set: the branch is a closed `if`/`elif` chain, not a fallthrough. A run that is still running elsewhere (another process) opens an empty live stream that heartbeats until its events arrive or it is reaped.
...
- **Known gap — `stalled` is not in the server's terminal set.** `_TERMINAL` and `_is_terminal` (`backend/app/api/chat.py`) list `completed` / `failed` / `cancelled` only, while `Run.status`, `obs.TERMINAL_RUN_STATUSES` and the frontend all treat `stalled` as terminal. Consequences a client must handle: the live `run_status: stalled` event **is** delivered (`_finalize_failure` emits it), but the server does not close the stream on it and keeps heartbeating; and a client reconnecting to a stalled run whose in-process history has been evicted gets no synthesized terminal event at all, only pings. The frontend closes its own `EventSource` on `stalled`, which is why this is not user-visible today. Treat `stalled` as end-of-stream client-side and resolve the run from `GET /runs/{run_id}`. This is tracked as an implementation gap, not an intended contract.
...
Run lifecycle transitions. Emitted at start (`running`), on pause (`paused_hitl`), on each resume (`running` again), and at the end. Source: `backend/app/orchestrator/runner.py` — `_execute` emits `running` / `paused_hitl` / `completed` directly, and `_finalize_failure` emits whichever terminal status it was called with.

| Field | Type | Meaning |
|---|---|---|
| `status` | string | `running` \| `paused_hitl` \| `completed` \| `failed` \| `cancelled` \| `stalled` |

`stalled` is the heartbeat reaper's terminal status (`reap_stalled_runs`, `backend/app/ambient/execute.py`), delivered through the same `_finalize_failure` path as `failed` and `cancelled`. The frontend treats it as terminal alongside those two (`streamRun`, `frontend/src/api/client.ts`).

**`queued` never appears on the stream.** It is a `runs.status` value only: `create_run` inserts the row as `queued` before any recorder exists, and the first `run_status` event a subscriber sees is `running`, emitted once the run holds an admission slot. A client that needs to know a run is queued reads `GET /runs/{run_id}`.
