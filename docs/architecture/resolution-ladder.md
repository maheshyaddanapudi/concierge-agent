...
**Rung 3 — `custom_sub_agent`.** Failing rung 2, the first `active` custom sub agent (in `created_at` order — `cache.sub_agents()` guarantees this ordering, and rung-3 precedence relies on it) whose `skill_ids` include the skill wins. Its `cache.sub_agent_snapshot` — persona, workflow DAG, embedded skill snapshots — feeds `get_compiled_worker`, which compiles the DAG via `build_worker` and caches the compiled graph keyed by `(sub_agent_id, "{updated_at}:{skills_digest}", checkpointer)` — the digest covers each bound skill's definition hash and status, so a skill edited or toggled off after the first invoke recompiles the worker instead of being served from the cached graph (third reading).
