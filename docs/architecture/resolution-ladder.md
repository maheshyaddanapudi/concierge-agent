...
**Before any rung: the ambient allowlist.** `resolve_capability` opens with `_refuse_outside_ambient_allowlist(capability)`. On an ambient run whose `RunContext` carries a routine allowlist, the capability's `id` (and every `skill_id`) is looked up and matched against the allowlist by **id, name or `tool_key`** — the same three keys the catalog projection matches on, so the two agree by construction. Anything outside raises `ResolutionError("… is outside this routine's allowlist")` before a rung is considered. A capability kind the allowlist does not mention is unconstrained, and a non-ambient run skips the check entirely.
...
**Rung 3 — `custom_sub_agent`.** Failing rung 2, the first `active` custom sub agent (in `created_at` order — `cache.sub_agents()` guarantees this ordering, and rung-3 precedence relies on it) whose `skill_ids` include the skill wins. Its `cache.sub_agent_snapshot` — persona, workflow DAG, embedded skill snapshots — feeds `get_compiled_worker`, which compiles the DAG via `build_worker` and caches the compiled graph under the three-part key `(str(sub_agent_id), f"{updated_at}:{skills_digest}", id(checkpointer))` — where `skills_digest` is the first 16 hex characters of a SHA-256 over the sorted `(skill_id, definition_hash, status, updated_at)` tuples of the snapshot's skills, and the third element is the checkpointer's **object identity**, not its value. The digest is why a skill edited or toggled off after the first invoke recompiles the worker instead of being served from the cached graph (third reading). A snapshot whose `sub_agent.id` is `None` — every ephemeral rung-4 worker — is never cached at all.
