...
| M53 | ✅ done | `docs/acceptance/prod/M53/` — a rolling deploy on the live model with three open streams and runs in flight (readiness 503 within 0.5 s of `SIGUSR1`, every stream reconnected with `Last-Event-ID` and resolved from the record with zero duplicates, every run terminal and truthful — one completed inside the drain, two cancelled naming the shutdown — leadership re-acquired 5 s into the new process), the readiness/liveness/polite-close transcript, retention seeded and purged behind its gates with the protected rows surviving, the incident signals on `/metrics` with the two dashboards screenshotted under load, an MCP server killed and reconnected on its own and a broken one tripping the breaker, a LISTEN backend terminated and repaired, the spend ceiling refusing a chat (429) and holding an ambient fire, the restore drill with the measured RTO (10 s), a browser stream reopened through the proxy's 502s during a roll, 44 contract tests |
| M54 | ✅ done | `docs/acceptance/prod/M54/` — three replicas cold-booting on an empty volume (one migrates, the others wait; tools once; a stub registered on one replica connected by the others in 3.3 s; a registry write moving `generation` and `dirty` on every `/cache/status`), a run created on A cancelled from B in 117 ms with C's foreign stream resolving from the record on the live model, a delivery fanned to three pinned streams and two browsers on different replicas (`watchers: 3` on the leader, pursuit held; webhook sent once nobody watched), the job clock once per interval across three logs and unchanged across a restart, one rate-limit budget across replicas with auth on, load at N=3 vs N=1 within the published budget (peak 65 of 97 connections), recall from 10k to 1M with the vector leg flat and the lexical leg named as the cost, 39 contract tests; four defects found by the drills and fixed (row-by-row sweeps OOM-looping a replica, unindexed self-references stalling deletes, 64 MB `/dev/shm`, a limiter test resetting the wrong store) |
| M55 | ✅ done | `docs/acceptance/prod/M55/` — the `AuthProvider` port + registry with the builtin behind it; the reference stub selected by environment on the shipped image enforcing a rule the builtin lacks (rows shared by tenant, writes gated on `editor`) through the middleware, the stores, both SSE streams and memory recall with a run on the live model; the contract suite over every registered provider (28 seam tests); the structural guard that no file outside `app/auth/` reads the switch; byte-identity with the default provider (the full suite unchanged); `docs/extending.md`; spec §20. The drill found the run stream leaking across tenants (an M34 gap) — fixed with the ambient stream alongside |
| M56 | ✅ done | `docs/acceptance/prod/M56/` — LICENSE (MIT), CONTRIBUTING, SECURITY (auth-is-a-fork stated), issue/PR templates, CHANGELOG reconstructed M13–M55 under `v1.0.0`, versions 1.0.0, README retold as run-and-extend; the §14 script (steps 1–11) on a fresh `docker compose up` of the release images against the live model with UI frames, an addendum for the DAG error edge and the agentic todo list, the M49 load scenarios re-run as the performance record, the suites on the release tree (1068 / 94); no git tag by decision — no consumer yet, the version lives in the files |
