...
2. **Milestone discipline** (spec §12). Work proceeds strictly milestone by milestone, and a milestone does not start until the previous one's tests are green. The POC core was M1 (registries + seed) → M2 (MCP manager) → M3 (worker factory) → M4 (orchestrator + SSE + observability) → M5 (admin UI) → M6 (test/compose polish); the project has since run to **M56**, through invocation and cache (M7, M9–M12), memory (M13–M18), ambient (M19–M30), completeness and hardening (M31–M35), the acceptance ceremony (M36), A2A (M37–M39), config and salience (M40–M43), forgetting and learners (M44–M47), switchability (M48) and the production-hardening plan (M49–M56, `docs/research/prod_hardening/PLAN.md`). New work continues the same way: a milestone or a named wave, scoped by spec sections, tests first, evidence at the end. The per-milestone table is in [README.md](../../README.md#milestone-status).
3. **Per-milestone loop** (from `CLAUDE.md`): restate the milestone's scope by listing the spec sections it implements → write tests first → implement until green → run the executable steps of the Acceptance Demo Script (spec §14) → update `README.md` (milestone status table + layout changes) and `CHANGELOG.md` → one conventional commit per coherent change → checkpoint summary, then continue.
