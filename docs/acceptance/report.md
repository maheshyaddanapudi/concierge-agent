...
2. **The ambient tick stalled silently once.** After stage 28 flipped the Ambient master switch off and on from the Settings page and edited the tick interval, the leader tick stopped flushing deliveries for about seven minutes: seven tier-0 rows sat undelivered, no `ambient_tick_failed` or `ambient_leader_*` line was logged, and `pg_locks` held no advisory lease. `docker compose restart backend` flushed all seven within one tick. A `PATCH /settings` off/on did not reproduce it. Stage 29 now runs a preflight probe so a stalled tick is named rather than misread as a held delivery.
