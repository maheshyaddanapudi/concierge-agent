...
| `recall.md` + `recall.json` | §14q-95: 10k → 100k → 1M embeddings on the typed `emb_64` HNSW column. The **vector leg is flat** — `Index Scan on memory_embeddings` at a plan cost of 274 / 249 / 277 — and the endpoint is not: c=1 p50 **49 ms → 209 ms → 1,221 ms**, because the lexical leg ranks every row matching any query term (`ts_rank_cd`) and the synthetic corpus matches 37.5% of the table (375k rows at 1M, 1.29 s on its own — the whole of the endpoint's p50). Zero errors at every level and concurrency. The 900k-vector bulk index build took 3,591 s on two workers; the database container peaked at ~860 MiB |
| `tests.md` | the M54 contract suite (39) and the full suites, executed: backend **1040 passed, 1 skipped**, frontend **94 passed**, ruff / mypy / doclint / prompt golden sets clean |
