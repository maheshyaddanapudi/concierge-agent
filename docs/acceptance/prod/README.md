...
| `M56/` | `m56-ceremony.sh`, `m56-addendum.sh`, `perf-record.sh` | the §14 acceptance script steps 1–11 on a fresh `docker compose down -v && up` (the third-reading image; the migration chain from the initial schema) with the UI after each step, the direct-invocation error-edge and agentic-todo addendum, and the performance record |
