...
| `prod/M49/` … | The production-hardening program (`docs/research/prod_hardening/PLAN.md`), one directory per stage: M49 holds the load **baseline** captured before any fix (`baseline.json` / `baseline.md`, the harness in `experiments/load/`), the prompt golden-harness proof including a deliberate regression, and the ruff BLE/S triage record; M50 holds the same scenarios re-run after the ceiling fixes (60 streams healthy, `/runs` flat 1k→10k), the trigger-quarantine and timezone transcripts, and the three screenshots (§14m-67..69); M51 holds the fault-injection transcripts (§14n-70..76: hung provider → wall clock, 429 classified, admission 503/queued on the live model, SIGTERM drain + SIGKILL reap, Redis killed → Postgres, webhook retry → dead-letter), the 30-minute ambient soak with RSS samples, the live-model sample, and two screenshots; M52 holds the adversarial transcripts (§14o-77..82: a fence-escape payload fired at the live model, SSRF and redirect refusals, billion-laughs and oversized feeds, write-only MCP secrets, redacted errors, refused regexes) and the MCP page screenshot; M53 holds the deploy-and-operate drills (§14p-83..90: a rolling deploy with open streams and in-flight runs on the live model, readiness/liveness/polite close, retention behind its gates, the incident signals with dashboard screenshots, MCP reconnect and circuit breaker, LISTEN repair, the spend ceiling across run kinds, the restore drill with its RTO) and the Settings/Runs/Chat screenshots; M54 holds the horizontal-scale drills at three replicas (§14q-91..96: a concurrent cold boot, a cross-replica cancel on the live model, delivery fan-out to every replica with two browsers, the cluster job clock, the distributed limiter, load at N=3 against N=1, recall from 10k to 1M embeddings) and the two toast screenshots; M55 holds the fork-seam drill (§14r-97..99: the reference auth provider selected by environment on the shipped image, its tenancy rule held end-to-end with a run on the live model, the first pass that found the run stream leaking across tenants, and the tests page); M56 holds the `v1.0.0` release ceremony — the §14 script on a fresh `docker compose up` of the release images with UI frames, its addendum, and the performance record |
