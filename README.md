...
> **Status: all milestones complete (M1–M36).** The core POC (M1–M6) plus the memory (M13–M18), ambient (M20–M25), and completeness/hardening waves (M26–M35) are done, and the **M36 full acceptance ceremony (spec §18.10) passed on a fresh `docker compose up`**: all eleven §14 steps, the §14c ambient UI + decision-plane additions, a live custom-gateway smoke, and the §11 byte-identity suites — evidence in [docs/acceptance/ceremony_m36/](./docs/acceptance/ceremony_m36/report.md). Backend + admin UI live; full backend suite **691 passed, 1 skipped**. Full manual-UI acceptance evidence — including the original single-pass Sonnet-5 campaign covering both orchestrator modes × thinking on/off, all four themes, HITL approve/deny/queue, failure/retry, and every admin surface — lives in [docs/acceptance/](./docs/acceptance/README.md). A fresh 1:1 parity re-run of that whole campaign on real `qwen3.8-max`, plus new full lifecycle sweeps of Memory (§16) and Ambient (§17/§18/§8.9), lives in [docs/acceptance_ambient/](./docs/acceptance_ambient/report.md). `spec.md` is the single source of truth; implementation proceeded milestone by milestone via spec-driven development (see `CLAUDE.md`).
...
- **[Acceptance evidence — memory + ambient](./docs/acceptance_ambient/report.md)** — a fresh 1:1 parity re-run of the base campaign plus full lifecycle sweeps of Memory (§16) and Ambient (§17/§18/§8.9), real LLM throughout, with honest findings on quirks hit along the way.
