...
export class A2uiMessageProcessor {
    ...
    buildNodeRecursive(baseComponentId, surface, visited, dataContextPath, idSuffix = '') {
        ...
        if (isObject(unresolvedProperties)) {
            for (const [key, value] of Object.entries(unresolvedProperties)) {
                resolvedProperties[key] = this.resolvePropertyValue(value, surface, visited, dataContextPath, idSuffix, key === 'action');
            }
        }
    ...
    resolvePropertyValue(value, surface, visited, dataContextPath, idSuffix = '', isInsideAction = false) {
        ...
        if (Array.isArray(value)) {
            return value.map(item => this.resolvePropertyValue(item, surface, visited, dataContextPath, idSuffix, isInsideAction));
        }
